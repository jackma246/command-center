    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-gray-900 rounded-xl p-5 border border-gray-800">
          ...
          <p className="text-3xl font-bold">${Number(data.monthToDate ?? 0).toFixed(2)}</p>
        ...
        <div className="bg-gray-900 rounded-xl p-5 border border-gray-800">
          ...
          <p className={`text-3xl font-bold ${(data.projectedMonthly ?? 0) > (data.monthlyBudget ?? 50) ? "text-red-400" : "text-green-400"}`}>
            ${Number(data.projectedMonthly ?? 0).toFixed(2)}
          </p>
        ...
        <div className="bg-gray-900 rounded-xl p-5 border border-gray-800">
          ...
          <p className="text-3xl font-bold">${Number(data.dailyAverage ?? 0).toFixed(2)}</p>
      ...
      <div className="bg-gray-900 rounded-xl p-5 border border-gray-800 mb-6">
        <div className="flex justify-between items-center mb-2">
          ...
          <span className="text-sm text-gray-400">{Number(data.budgetUsedPercent ?? 0).toFixed(1)}%</span>
        </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {(data.costs ?? []).map((c, i) => (
                <tr key={i} className="border-t border-gray-800">
                  ...
                  <td className="py-3 text-right font-mono">${Number(c.cost ?? 0).toFixed(2)}</td>
            ...
            <tfoot>
              <tr className="border-t border-gray-700 font-semibold">
                ...
                <td className="pt-3 text-right font-mono">${Number(data.monthToDate ?? 0).toFixed(2)}</td>
